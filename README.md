...
- Folders are a tree: Study › Math, Study › Science, Land, Personal, up to four levels. Browse them on the Folders screen (Documents › folder icon, or Settings), create, rename and delete folders (documents move to the parent), and file documents from the document screen or a selection. A folder can be locked with a 6-digit PIN: everything inside it and below it disappears from lists, search, Home and previews until the PIN is entered, and locks again after a minute in the background or on "Lock now". The PIN is a salted hash in the Android Keystore (expo-secure-store), never stored as typed, with a doubling wait after five wrong tries. There is no recovery by design; setup says so and requires an acknowledgement, and "Forgot your PIN?" offers only a reset that erases the locked documents.
- Live scanner: corner positions are median-filtered over three frames before smoothing, so a single bad detection cannot jog the overlay or restart the stability timer; an automatic capture focuses and meters on the page first (bounded to 450 ms) and takes the sensor's highest-resolution still.
- Folders (older note): a document carries one folder label (Receipts and Study are assigned by their presets). The Documents tab shows a strip of folders in use with counts and filters by one; the document screen has a folder chip; a selection can be moved to a folder or a new one typed on the spot. Folders exist exactly while a document is in them, so there is nothing to tidy.
