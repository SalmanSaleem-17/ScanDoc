...
| Advertising (Google AdMob) | Native card at the end of Home, Documents, Tools and Settings; after finished tasks; rewards in Settings and inside tools | Consent first: Google's UMP form runs before any ad request and the SDK is initialised only if ads may be requested; declining starts nothing and loses nothing. One native ad drawn in the app's own card style as the last item of each tab screen (no banner); an interstitial only after a workflow task completes, never in the first 90 seconds of a session, at most every 3 minutes and 6 per session; an app-open ad only when returning after 3 or more minutes away, never on a cold start, never after system UI the app opened itself; never over the camera or the page editor. Ad content is capped at rating G. Development builds use Google's test units. Rules in src/features/ads/policy.mjs, unit tested. The module is optional so Expo Go keeps working without it. |
| Rewards (one rewarded video unit) | Settings → Ads & rewards; the draft screen; inside premium tools | Three things a short video buys, all local to the device and never sold: 15 ad-free minutes (stacks when repeated); no PDF watermark for 24 hours; and one premium tool (OCR, Merge, Split, PDF to Images, Compress PDF, Compare) for 24 hours. New PDFs otherwise carry a small "Scanned with ScanDoc" label drawn onto each page image. A tool's unlock gate appears only when a video can actually be shown; in Expo Go, without consent or with no fill the tool is simply open. Rules in src/features/ads/rewards.mjs, unit tested. |
